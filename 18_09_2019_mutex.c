...
*/
        ...
        if(pthread_mutex_lock(ready+me)) {
            perror("error mutex lock thread");
            exit(EXIT_FAILURE);
...
*/
        ...
        if(pthread_mutex_lock(ready+thread_number)) {
            perror("error mutex lock trhead");
            exit(EXIT_FAILURE);
...
*/
    ...
    ready = malloc(sizeof(pthread_mutex_t)*(thread_number+1));
